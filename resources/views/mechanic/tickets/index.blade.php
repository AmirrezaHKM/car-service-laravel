            ...
            @foreach($tickets as $ticket)
                <tr class="border-b hover:bg-gray-100">
                    ...
                    <td class="py-3 px-4 text-right flex gap-2 justify-end">
                        <a href="{{ route('mechanicpanel.tickets.show', $ticket) }}" class="bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700 transition">جزئیات</a>

                        <form action="{{ route('mechanicpanel.tickets.destroy', $ticket) }}" method="POST" onsubmit="return confirm('آیا از حذف این تیکت مطمئن هستید؟');">
                            @csrf
                            @method('DELETE')
